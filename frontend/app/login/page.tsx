      ...
      <div className="w-full max-w-[440px] bg-relay-surf rounded-[14px] border border-relay-border overflow-hidden shadow-sm">
        ...
        <div className="bg-relay-subtle px-[18px] py-[14px] border-b border-relay-border flex items-center justify-between">
          ...
          <div className="inline-flex items-center gap-[5px] bg-relay-subtle border border-relay-border-strong rounded-full px-[10px] py-[3px] text-[11px] text-relay-text-secondary">
            <span className="w-[6px] h-[6px] rounded-full bg-relay-accent-alt"></span>
            Secure login
          </div>
        </div>
        ...
        <div className="p-[20px]">
          ...
          <div className="flex border-b border-relay-border mb-[16px]">
            ...
            <Link href="/register" className="flex-1 py-[8px] text-center text-[12.5px] font-medium text-relay-text-secondary border-b-2 border-transparent transition-colors hover:text-relay-text-primary">
              Register
            </Link>
          </div>

          <p className="text-[13px] font-medium text-relay-text-primary mb-[14px]">Welcome back</p>
          ...
          <form onSubmit={handleSubmit(handleLogin)} className="space-y-[14px]">
            {/* Email */}
            <div className="space-y-[4px]">
              ...
              <div className="relative">
                ...
                />
                <div className="absolute right-[12px] top-1/2 -translate-y-1/2 text-relay-accent">
                   <svg width="13" height="13" viewBox="0 0 16 16" fill="none"><path d="M14 3H2a1 1 0 0 0-1 1v8a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1V4a1 1 0 0 0-1-1Z" stroke="currentColor" strokeWidth="1.2"/><path d="M1 4l7 5 7-5" stroke="currentColor" strokeWidth="1.2"/></svg>
                </div>
              </div>
            ...
            <div className="space-y-[4px]">
              <div className="flex justify-between items-center">
                ...
                <Link href="/forgot-password" size={14} className="text-relay-accent text-[11px] hover:underline">Forgot password?</Link>
              </div>
              <div className="relative">
                <input
                  {...register("password")}
            ...
            {/* Remember Me */}
            <div className="flex items-center gap-[7px] mb-[16px] px-1">
              <input
                {...register("rememberMe")}
                type="checkbox"
                id="rememberMe"
                className="w-[14px] h-[14px] appearance-none border border-relay-border-strong rounded-[4px] bg-relay-input checked:bg-relay-accent checked:border-relay-accent cursor-pointer transition-all flex items-center justify-center relative after:content-['✓'] after:text-white after:text-[10px] after:hidden checked:after:block"
              />
              <label htmlFor="rememberMe" className="text-[11.5px] text-relay-text-secondary cursor-pointer">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-relay-btn text-relay-btn-text rounded-[9px] py-[10px] text-[13px] font-medium flex items-center justify-center gap-[6px] cursor-pointer hover:bg-relay-accent-alt transition-all disabled:opacity-70 disabled:cursor-not-allowed group shadow-sm"
            >
              {isLoading ? (
